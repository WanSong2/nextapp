...
const getFareData = async ({ pageParam }) => {
  try {
...
export async function fetchItems({ pageParam }) {
  return new Promise(async (resolve) => {
    try {
      const data = await getFareData({ pageParam });
      resolve({
        data,
        currentPage: pageParam,
        nextPage: pageParam + LIMIT < data.TOTALPAGE ? pageParam + LIMIT : null
      });
    } catch (error) {
